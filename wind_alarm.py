...
class WindAlarm(StdService):
    ...
    def newArchiveRecord(self, event):
        ...
        record = event.record
        eval_expresion = eval(self.expression, None, record) 
        ...
        if not self.last_msg_ts or abs(time.time() - self.last_msg_ts) >= self.time_wait:
            ...
            try:                                                              # NOTE 2
                ...
                if eval_expresion:                       # NOTE 3
                    syslog.syslog(syslog.LOG_INFO, "VIENTO ----- eval True")
                   
                    # Verifico si el tiempo acumulado de mi condicion de alarma es igual o superior a self.time_event
                    if abs(time.time() - self.last_event_time) >= self.time_event:
                        syslog.syslog(syslog.LOG_INFO, "VIENTO ----- time event OK")
                        # Sound the alarm!
                        # Launch in a separate thread so it doesn't block the main LOOP thread:
                        syslog.syslog(syslog.LOG_INFO, "VIENTO ----- pre Thread")
                        t = threading.Thread(target = WindAlarm.soundTheAlarm, args=(self, record))
                        t.start()
                        syslog.syslog(syslog.LOG_INFO, "VIENTO ----- post Thread")
    ...
    def soundTheAlarm(self, rec):
        ...
        status = self.in_event
        # Get the time and convert to a string:
        t_str = timestamp_to_string(rec['dateTime'])

        # Log it in the system log:
        log_info = "-- WIND -- alarm: Alarm expression '{}' evaluated {} at {}".format(self.expression, status, t_str)
